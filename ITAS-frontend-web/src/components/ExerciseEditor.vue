<template>
  <div class="form-container">
    <h2 class="form-title">{{ isEditMode ? "编辑练习" : "新建练习" }}</h2>
    <form>
      ...
      <div class="form-group">
        <label for="description">练习说明</label>
        <textarea
          id="description"
          name="description"
          placeholder="请输入练习说明"
...
export default {
  ...
  mounted() {
    console.log("触发加载习题方法");
    this.loadExerciseData();
  },
  methods: {
    async loadExerciseData() {
      try {
        const response = await get_exercise_questions(this.exerciseId);
        ...
        if (exerciseData.questions && exerciseData.questions.length > 0) {
          this.questions = exerciseData.questions.map((q) => ({
            id: q.id,
    ...
    async submitExercise() {
      ...
      try {
        if (this.isEditMode) {
          ...
          await update_exercise(updateData);
          this.$message.success("练习发布成功");
          this.$emit("success", this.exerciseId);
        } else {
          // 新增模式 - 创建练习
          const data = {
          ...
          const response = await create_exercise(data);
          ...
          await add_exercise_questions(params);
          this.$message.success("练习新建成功");
        }

        this.$emit("success");
        this.resetForm();
      } catch (error) {
        console.error(`${this.isEditMode ? "更新" : "新建"}练习失败:`, error);
        this.$message.error(
          `${this.isEditMode ? "更新" : "新建"}练习失败，请稍后重试`
        );
      }
    },
