...
<script>
import { update_course } from "@/http/api.js";
export default {
  name: "UpdateCourse",
  data() {
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        data.append("course_id", this.$route.params.courseId);
        ...
        const response = await update_course(data);
        if (response.code === 200) {
          this.$message.success("课程更新成功！");
          this.$router.push("/teacher_profile");
        } else {
          this.$message.error("课程更新失败，请重试。");
        }
      } catch (error) {
